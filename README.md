...
## 8. 收集表单数据和表单的前台验证
    ...
    2). 操作表单数据
        ...
        form.getFieldValue('标识名称'): 根据标识名称得到对应字段输入的数据
        form.resetFields(): 清除输入框的缓存数据
...
## 4. 显示二级分类列表
    ...
    -“查看子分类”标签的点击事件问题：
        onClick={this.showSubCategories(category)}  加了括号后会在初始渲染之后就立刻调用， 需要在点击事件后调用，所以使用
        onClick={()=>{this.showSubCategories(category)}}  在外层再包一层函数，可以避免此问题
    -如何向事件回调函数传入参数：
        先定义一个匿名函数 ()=>{}，在函数中调用事件回调函数并传入数据
